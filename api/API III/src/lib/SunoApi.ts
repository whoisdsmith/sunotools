...
class SunoApi {
  ...
  private solver: any; // TODO: Implement proper solver interface
  private ghostCursorEnabled = yn(process.env.BROWSER_GHOST_CURSOR, { default: false });
  private cursor?: Cursor;
  private captchaDir: string = path.join(process.cwd(), 'captchas');
  ...
  */
  ...
  private async launchBrowser(): Promise<BrowserContext> {
    ...
    const browser = await this.getBrowserType().launch({
      args,
      headless: false // Force browser to open in non-headless mode for CAPTCHA solving
    });
